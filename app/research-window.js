...
/* 证据检证已独立成工具窗：从研究台一键打开（spawn 独立进程） */
ipcMain.handle('research:openVerify', () => {
  try {
    const { spawn } = require('child_process');
    spawn(process.execPath, [path.join(__dirname, 'tool-window.js'), '--tool=verify'], { cwd: ROOT, windowsHide: false, stdio: 'ignore', detached: true }).unref();
    return { ok: true };
